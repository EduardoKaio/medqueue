...
import com.queueflow.queueflow.dto.ErrorResponse;
import com.queueflow.queueflow.service.admin.InsightsService;
...
public class InsigthsController {
    ...
    @GetMapping("/users-por-especialidade")
    @Operation(summary = "Obter quantidade de users por especialidade")
    public ResponseEntity<?> obterUsersPorEspecialidade() {
        try {
            Map<String, Integer> resultado = insightsService.calcularUsersPorEspecialidade();
            return ResponseEntity.ok(resultado);
        } catch (IllegalStateException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.internalServerError()
                .body(new ErrorResponse("Erro interno ao calcular users por especialidade."));
        }
    }
